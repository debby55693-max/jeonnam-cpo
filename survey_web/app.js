...
function selectItem(key) {
  ["bell", "self", "kit"].forEach(function(id) {
    const card = document.getElementById("itemCard_" + id);
    if (card) card.classList.remove("selected");
  });
  const target = document.getElementById("itemCard_" + key);
  if (target) target.classList.add("selected");
  const input = document.getElementById("selectedItem");
  if (input) input.value = _ITEM_LABELS[key] || key;
}
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function validateRequiredFields() {
    ...
    if (!selectedItem) {
      missing.push({ label: "지원 물품 선택 (1개 필수)", target: document.getElementById("itemCard_bell") });
    }
  ...
  async function initVworldMap() {
    try {
      ...
      // ── 주소 검색을 VWorld 지도 로딩과 독립적으로 먼저 바인딩 ──
      // VWorld 지도가 실패해도 주소 팝업 검색은 항상 동작하도록 합니다.
      window.__updateSelectedPointRef = function(lon, lat, sourceLabel) {
        selectedLon = lon;
        selectedLat = lat;
        setLocationMessage(
          "<b>" + escapeHtml(sourceLabel || "주소 선택") + "</b><br>" +
      ...
      async function searchAddressToCoord() {
        const rawQuery = (addressInput ? addressInput.value.trim() : "");
        const query = normalizeAddressKeyword(rawQuery);
        if (addressInput && query && addressInput.value !== query) addressInput.value = query;
        if (!query) { setResultMessage("주소를 먼저 입력해주세요."); return; }
        setResultMessage("주소 목록을 찾는 중입니다...");
        try {
          await searchAddressList(query, 1, window.APP_CONFIG?.VWORLD_API_KEY, window.__updateSelectedPointRef);
          ...
          console.error(err);
          setResultMessage("주소 검색 중 오류가 발생했습니다: " + err.message);
        }
      }

      bindAddressSearchModalEvents(window.APP_CONFIG?.VWORLD_API_KEY, window.__updateSelectedPointRef);
      if (searchAddressBtn) searchAddressBtn.addEventListener("click", searchAddressToCoord);
      if (addressInput) {
        addressInput.addEventListener("keydown", function(e) {
          if (e.key === "Enter") { e.preventDefault(); searchAddressToCoord(); }
        });
      }
      // ── 주소 검색 독립 바인딩 끝 ──
      ...
      // config.js에 등록된 VWorld 서비스 도메인을 사용 (현재 접속 URL과 달라도 동작)
      const domain = window.APP_CONFIG?.VWORLD_DOMAIN || window.location.origin;
      ...
      // 지도 핀 배치 기능이 준비됐으므로 포인트 핸들러를 업그레이드
      window.__updateSelectedPointRef = updateSelectedPoint;
        ...
        submitBtn.addEventListener("click", async function () {
          ...
          try {
            ...
            const payload = {
              ...
              requested_item: (document.getElementById("selectedItem")?.value || null),
